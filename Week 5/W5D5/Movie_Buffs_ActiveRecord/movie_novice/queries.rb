...
def pulp_fiction_actors
  ...
  Actor.joins(:movies).select('actors.id, actors.name').where('movies.title = ?', 'Pulp Fiction')
...
def uma_movies
  ...
  Movie.joins(:actors).select('movies.id, movies.title, movies.yr').where('actors.name = ?', 'Uma Thurman').order('movies.yr ASC')
end
